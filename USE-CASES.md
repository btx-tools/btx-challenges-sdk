...
| **Browser captcha** — you want a sub-second "click to admit" widget in front of users at `target_solve_time_s: 1-4s` | **❌ Not a casual captcha at `n=512`.** | Too slow on either path: WASM ~16 s at floor (8-worker pool); WebGPU is actually **slower** — ~860 ms/attempt vs WASM's ~128 ms (a full proof is hundreds of matmuls, and `n=512` caps GPU batching → ~11 min floor solve). WebGPU only wins at small/devnet `n`. See [`BROWSER-PERF-FINDINGS-2026-05-23.md`](./BROWSER-PERF-FINDINGS-2026-05-23.md). Use hCaptcha / Cloudflare Turnstile / Friendly Captcha. |
...
2. **WebGPU compute shader implementation** — **shipped** as [`@btx-tools/matmul-webgpu@0.1.0`](https://www.npmjs.com/package/@btx-tools/matmul-webgpu) (`Solver` `mode: 'webgpu'`), byte-exact. Its *single-matmul* microbenchmark is ~50× the WASM matmul, but a full proof at the live `n=512` is hundreds of matmuls, so end-to-end it's **slower than WASM** (measured ~860 ms/attempt vs ~128 ms). It wins only at small/devnet `n` (sub-ms at `n=64`). A future **fused/batched WebGPU kernel** or a smaller-`n` protocol variant could change this — until then WASM is the browser solver.
