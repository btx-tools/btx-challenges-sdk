...
**Root cause #2** (✅ closed in [Unreleased] / 0.0.4 via live HTTP roundtrip against btx-iowa): pure-JS proof shape is now algorithm-cross-validated against btxd's pinned goldens (byte-equal, 0.0.1) AND the full live `issue → Solver.solve({ mode: 'pure-js' }) → client.redeem` lifecycle passes end-to-end (421 s, `valid: true`). **If you're hitting `digest_mismatch` on 0.0.4+** even after upgrading and confirming the goldens pass, file an issue with `challenge_id` immediately — it's a real bug, not a known surface gap.
...
**B-3 (root cause #2 of digest-mismatch-byte-order) was previously listed here** — **closed in `[Unreleased]` / 0.0.4** via live-roundtrip lifecycle test against btx-iowa (paused mining, floor difficulty). Cookbook entry retained for historical context; no longer indicates an open risk.
