...
| [`@btx-tools/matmul-webgpu`](./packages/matmul-webgpu)           | **WebGPU matmul solver kernel** — byte-exact, GPU-accelerated; powers `Solver` `mode: 'webgpu'`. Fastest at small/devnet `n`; at the live `n=512` a full proof isn't faster than WASM yet (see USE-CASES). New (`0.x`). | [![npm](https://img.shields.io/npm/v/@btx-tools/matmul-webgpu)](https://www.npmjs.com/package/@btx-tools/matmul-webgpu)         |
...
> **Read [USE-CASES.md](./USE-CASES.md) before deciding to integrate.** This SDK is **server-side admission middleware** for chain-anchored proof-of-work gating. For production gating, solve with `mode: 'rpc'` against a dedicated non-mining btxd, or `mode: 'wasm'` for fast no-node solving (server/edge, CLI, high-friction gates). **For in-browser solving, WASM is the fastest path** (~128 ms/attempt at the live `n=512`); the new `mode: 'webgpu'` kernel is faster only at small/devnet `n` — at `n=512` a full proof is hundreds of matmuls, so it's **slower than WASM end-to-end** (measured ~860 ms/attempt). Neither is a sub-second captcha — treat browser use as one-shot / high-friction admission, or solve server-side.
